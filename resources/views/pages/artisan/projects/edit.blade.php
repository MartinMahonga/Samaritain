    ...
    <div class="bg-primary dark:bg-primary-700 text-white overflow-hidden">
        <div class="relative max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12 md:py-16">
            <div class="flex items-center gap-3 mb-4">
                ...
                <div>
                    <h1 class="text-3xl md:text-4xl font-bold text-white">Modifier la réalisation</h1>
                    <p class="text-white/90 dark:text-white/90">Mettez à jour les informations de votre projet</p>
                </div>
            </div>
        </div>
    </div>
                    ...
                    enctype="multipart/form-data" class="space-y-8" x-data="{
                        ...
                        handleFileSelect(event) {
                            ...
                            for (let i = 0; i < files.length; i++) {
                                this.newPreviews.push(URL.createObjectURL(files[i]));
                            }
                        }
                    }">
                    @csrf
                    ...
                    <div>
                        ...
                        <div class="space-y-5">
                            ...
                            @if ($project->images->isNotEmpty())
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Photos actuelles</label>
                                    <div class="grid grid-cols-2 md:grid-cols-3 gap-3">
                                        @foreach ($project->images as $image)
                                            <div class="relative group">
                                                <img src="{{ Storage::url($image->image_path) }}" alt="{{ $project->title }}"
                                                    class="w-full h-32 object-cover rounded-xl border border-gray-200 dark:border-gray-600 shadow-sm">
                                                <form action="{{ route('artisan.projects.image.destroy', [$artisan, $image->id]) }}" method="POST"
                                                    onsubmit="return confirm('Supprimer cette image ?')">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" class="absolute top-1 right-1 p-1 bg-red-500 dark:bg-red-600 text-white rounded-full hover:bg-red-600 dark:hover:bg-red-700 opacity-0 group-hover:opacity-100 transition-opacity">
