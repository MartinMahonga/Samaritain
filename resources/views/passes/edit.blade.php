...
@section('title', 'Modifier le Pass')
...
@section('content')
    <div class="max-w-7xl mx-auto">
        ...
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-gray-800 dark:text-white">Modifier le Pass de {{ $pass->holder_name }}</h1>
        </div>

        <div class="bg-sidebar dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 overflow-hidden">
            <div class="p-6">
                <form action="{{ route('passes.update', $pass) }}" method="POST" x-data="passForm()" @submit="validateForm">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        <!-- Colonne gauche -->
                        <div class="space-y-5">
                            <x-form.input label="Nom du titulaire" name="holder_name" x-model="holderName" value="{{ old('holder_name', $pass->holder_name) }}" />
                            <x-form.input label="Téléphone" type="tel" name="phone" icon="phone" value="{{ old('phone', $pass->phone) }}" />
                            <x-form.input label="Email (optionnel)" type="email" name="email" icon="mail" value="{{ old('email', $pass->email) }}" />
                            <x-form.input label="Nombre de visites autorisées" type="number" name="allowed_visits"
                                min="1" x-model="allowedVisits" @change="updateVisitsLabel" value="{{ old('allowed_visits', $pass->allowed_visits) }}" />
                        ...
                        <div class="space-y-5">
                            <x-form.input label="Date de début" type="date" name="start_date" icon="calendar"
                                x-model="startDate" @change="updateExpirationMin" value="{{ old('start_date', $pass->start_date->format('Y-m-d')) }}" />
                            <div>
                                ...
                                <div class="relative">
                                    ...
                                    <input type="date" name="expiration_date"
                                        value="{{ old('expiration_date', $pass->expiration_date->format('Y-m-d')) }}" required
                                ...
                                <div class="space-y-2 text-sm">
                                    <div class="flex justify-between">
                                        ...
                                        <span class="font-medium">
                                            ...
                                            @elseif($pass->is_expired)
                                                <span class="text-red-600 dark:text-red-400">Expiré</span>
                                            @else
                                                <span class="text-orange-600 dark:text-orange-400">Inactif</span>
                                            @endif
                                        </span>
                                    </div>
                                    <div class="flex justify-between">
                                        <span class="text-gray-600 dark:text-gray-400">Visites restantes :</span>
                                        <span class="font-medium text-gray-800 dark:text-white">{{ $pass->visits_remaining }}</span>
                                    </div>
                                    <div class="flex justify-between">
                                        <span class="text-gray-600 dark:text-gray-400">Créé le :</span>
                                        <span class="font-medium text-gray-800 dark:text-white">{{ $pass->created_at->format('d/m/Y') }}</span>
                                    </div>
                                </div>
                            </div>
    ...
    <script>
        function passForm() {
            return {
                holderName: '{{ old('holder_name', $pass->holder_name) }}',
                allowedVisits: {{ old('allowed_visits', $pass->allowed_visits) }},
                startDate: '{{ old('start_date', $pass->start_date->format('Y-m-d')) }}',
                expirationDate: '{{ old('expiration_date', $pass->expiration_date->format('Y-m-d')) }}',
